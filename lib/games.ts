// Mock data ported from the reference prototype. In references/templates the
// stylesheet-and-data files are shuffled: this content lives in the file named
// `biblioteca.jsx` there, even though it is the prototype's `data.jsx`.
...
/** Deterministic fake leaderboard. Same LCG as the prototype, so a given
 *  seed always yields the same rows on server and client. */
